...
const verifiedSales = '5,000,000+ copies sold';
...
function statCards() {
  ...
  <article class="mc-card mc-stat rounded-xl p-6"><span class="font-label-caps text-label-caps text-on-surface-variant">Verified milestone</span><strong class="mt-2 block font-headline-lg text-headline-lg text-paint-yellow">${verifiedSales}</strong><p class="mt-2 text-sm leading-relaxed text-on-surface-variant">Sales milestone retained as owner-verified; not a live counter.</p></article>
...
function compliance() {
  return `<section class="mc-compliance-strip" aria-label="Source and affiliation note"><strong>Unofficial guide.</strong> Not affiliated with Steam, Valve, or the game developers. No downloads are hosted here.<span>Source note: Steam store pages, public player-facing materials, and owner-verified sales milestone; gameplay advice is original and source-labeled.</span><span>Last checked: ${lastChecked}.</span></section>`;
}
...
function pageHtml(page) {
  ...
  return `${head(page)}<body class="min-h-screen flex flex-col font-body-md overflow-x-hidden">${nav()}${mobileMenu()}<main class="mx-auto flex w-full max-w-container-max flex-grow flex-col gap-12 px-margin-mobile py-8 md:px-margin-desktop md:py-12">
